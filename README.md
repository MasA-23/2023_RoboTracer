...
- モータref電圧の上昇
  ...
  　ステッピングモータは電圧をかけることでトルクと最高速度が上がるため，モータへかける電圧を上げました．電源電圧を上げる、またはref電圧を上げるといった方法が挙げられます。前者はコンデンサの交換等が必要となるため、改良が容易な後者を選択をしました．
...
## 6.制御方法
 　以前は、ラインセンサの値を2値化して制御していましたが、より滑らかな走行にするためにPD制御を導入しました。PD制御を行うには偏差eを求める必要があります．ラインセンサのアナログ値を左から L5，L4，L3，L2，L1，R1，R2...R5 とし，センサのアナログ値には距離が離れたセンサほど重みをつけるため，定数 k1，k2...k5 をかけます．
...
1分で平均°のズレは誤差と言えると思います。

   追記
   ステッピングモータを使用しているため、左右のステップ数から角度を算出する事ができます。
