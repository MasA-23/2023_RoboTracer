...
第40回北陸信越地区大会の試走会において、直線で加速しない問題が発生しました。ログを見てみると、直線区間の中央で取得した角速度が大きいため、曲線として認識されていました。
...
トレーサの速度を上げていくと、タイヤのスリップや計算誤差などにより、ロボットが計算した走行距離と実際の走行距離がずれていきます。その結果、まだ曲線なのに加速してコースアウトしてしまいます。そのため、距離補正プログラムを作成しました。

### 補正方法

補正にはマーカを使います。1次走行でマーカ反応時の走行距離を保存し、2次走行でマーカ反応時に1次走行で保存した距離に補正するというものです。

### 補正マーカ

曲率の小さい区間や極端に短い区間では、誤反応により正しく補正が行われない可能性があります。そのため、R10と区間距離20cm以下の区間以外のマーカで補正を行うようにしました。

### 補正区間

補正するマーカの前後5cmでマーカセンサが反応したら
